$(function () {
	function PrintHistoryViewModel(parameters) {
		...
		self.pureData = {}
		self.lastMonthGraphMinimum = ko.observable(
			moment(new Date()).subtract(1, 'months').valueOf()
		)
		...
		self.formatFilament = function () {
			...
			if (this.filamentLength() != undefined) {
				tool0 += formatFilament({
					length: this.filamentLength(),
					volume: this.filamentVolume(),
				})
			}
		...
		self.changeGraphRange = function (range) {
			if (range == 'week') {
				self.lastMonthGraphMinimum(
					moment(new Date()).subtract(1, 'weeks').valueOf()
				)
			} else if (range == 'month') {
				self.lastMonthGraphMinimum(
					moment(new Date()).subtract(1, 'months').valueOf()
				...
				self.lastMonthGraphMinimum(
					moment(new Date()).subtract(1, 'quarter').valueOf()
				)
			}
		...
		function printhistoryLabelFormatter(label, series) {
			return (
				"<div style='font-size:8pt; text-align:center; padding:2px; color: #666666;'>" +
				label +
				'<br/>' +
		...
		self.updatePlots = function () {
			...
			_.each(_.keys(self.pureData), function (key) {
				var day = moment
					.unix(self.pureData[key].timestamp)
					.hour(0)
					.minute(0)
					.second(0)
					.millisecond(0)
		...
		self.onSettingsBeforeSave = function () {
			self.global_settings.settings.plugins.printhistory.spool_inventory(
				self.spool_inventory.slice(0)
			)
		}
		...
		self.showDetailsDialog = function (selectedData) {
			if (self.detailsDialog) {
				self.itemForEditing(
					new HistoryItem(ko.mapping.toJS(selectedData))
				)
		...
		self.sortOrder = function (orderType) {
			...
			if (orderType == 'fileName') {
				order =
					self.listHelper.currentSorting() == 'fileNameAsc'
						? '(' + _('ascending') + ')'
				...
				order =
					self.listHelper.currentSorting() == 'timestampAsc'
						? '(' + _('ascending') + ')'
				...
				order =
					self.listHelper.currentSorting() == 'printTimeAsc'
						? '(' + _('ascending') + ')'
