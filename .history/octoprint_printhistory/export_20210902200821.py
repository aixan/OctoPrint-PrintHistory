...
def exportHistoryData(self, exportType):
    ...
    if history_dict is not None:
        ...
        headers = ['File name', 'Timestamp', 'Success', 'Print time',
                   'Spool', 'Filament length', 'Filament volume', 'User']
        fields = ['fileName', 'timestamp', 'success', 'printTime',
                  'spool', 'filamentLength', 'filamentVolume', 'user']
        if exportType == 'csv':
            ...
            for historyDetails in history_dict:
                ...
                if historyDetails.get("Build time", None):
                    match = re.match(r"(\d+) hours (\d+) minutes",
                                     historyDetails.get("Build time", None))
                    historyDetails["Build time"] = (
                        int(match.group(1)) * 60 + int(match.group(2))) * 60
                parameters_row = ParametersRow(**prepare_dict(historyDetails))
                writer.writerow([getattr(parameters_row, field)
